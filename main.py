            ...
            for symbol in list(pairs):
                ...
                try:
                    ...
                    if response.status_code != 200 or not response.text.strip():
                        print(f"⚠️ API error or empty response for {symbol}: HTTP {response.status_code}")
                        continue

                    try:
                        raw = response.json()
                    except Exception:
                        print(f"⚠️ Invalid JSON for {symbol}: {response.text[:200]}")
                        continue

                    if "values" not in raw:
                        print(f"⚠️ No data for {symbol}: {raw}")
                        continue
