...
# Generic fetch used by BOTH call sites (main 1h fetch + higher-timeframe 4h fetch) —
# parameterized so each site's own interval/outputsize/dp is preserved exactly, while
# both share the same rotation/failover state above.
def fetch_twelve_data(symbol, interval, outputsize, dp):
        ...
        while True:
            ...
            if elapsed < 60 * 60:
                for symbol in list(pairs):
                    ...
                    raw = fetch_twelve_data(symbol, interval="1h", outputsize=1000, dp=2)
                    ...
                    if last_rsi is None or ema_20 is None or macd is None:
                        ...
                        def get_higher_timeframe_bias(symbol):
                            ...
                            try:
                                raw_htf = fetch_twelve_data(symbol, interval="4h", outputsize=100, dp=2)
