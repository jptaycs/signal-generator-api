...
4. A signal fires by majority vote (4 of 7 indicators agreeing) **and** a trend-confirmation guard: at least one of MACD/ADX must also agree with that majority's direction, or the candidate signal is downgraded to HOLD. This exists to stop mean-reversion indicators (RSI, Stochastic, Bollinger, CCI) from outvoting a fully absent or conflicting trend, without requiring both trend indicators to agree simultaneously. On corrected price data (see the `dp=5` note below), this combination fires on roughly 3.5–8.5% of bars per pair — verified empirically, not just by threshold reasoning.
